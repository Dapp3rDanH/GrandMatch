# -*- coding: utf-8 -*-
"""
Triangulation_Universal_V2.py compares individuals' DNA files. Any number
of individuals can be compared to each other at the same time. The resulting 
matching segments are, by definition, triangulated.

FILES_PATH: Path to folder where the DNA files are stored. 

MAP_PATH: Path to folder containing min_map.txt.

TRIANG_NO: The number of individuals to be compared to each other at the same 
time.
...
V2 has a GEDmatch-like output. 

© Mick Jolley (mickj1948@gmail.com) 2024
...
# The number of individuals to be compared to each other at the same time.
TRIANG_NO = 3
...
def conditions(al1x, al2x, al1y, al2y):
    if (al1x == al2x and al1y == al2y and al1x != al1y):
        return 0
    elif (al1x == al1y and al2x == al2y) or \
        (al1x == al2y and al2x == al1y):
        return 2
    else:
        return 1
...
def scan_genomes(dx, dplot, dpairs):
    # vector = *p
    vpairs = list(combinations(pair,2))
    dmcols = []
    for pr in vpairs:
        dmcols.append(f"{pr}-match")
        dm[f"{pr}-match"] = np.vectorize(conditions)(
            dm["{}-allele1".format(pr[0])],
            dm["{}-allele2".format(pr[0])],
            dm["{}-allele1".format(pr[1])],
            dm["{}-allele2".format(pr[1])],
        )
        
    dm['zero'] = (dm[dmcols] == 0).sum(axis=1) 
    dm['one'] = (dm[dmcols] == 1).sum(axis=1) 
    dm['two'] = (dm[dmcols] == 2).sum(axis=1) 
            
    dm['match'], dm['color'] = np.vectorize(condx)(dm['zero'], dm['one'], dm['two'], 
                                      len(vpairs))
    ...
    for i in range(len(dm)):
        if i == 0 and dm.loc[i, "match"] > 0:
            nsnps = 1
            segflag = True
            stpos = dm.loc[i, "position"]
        elif not segflag and dm.loc[i, "match"] > 0:
            nsnps = 1
            segflag = True
            stpos = dm.loc[i, "position"]
    ...
    dg = pd.DataFrame(data = dm['position'])
    dg[pname] = dm['color']
    ...
    if len(dplot) == 0:
        ...
        dplot = pd.merge(dplot, dg, on=("position"))
        dpairs.append(pair)

    print(f"\nChromosome {CHROMOSOME} " + pname + " comparison completed.")
...
def display_matches():
    ...
    img1.rectangle([(200, 30), (250, 50)], fill="yellow")
    img1.text((110, 32), "FIR", font=fnt, fill="black")
    img1.text((260, 32), "HIR", font=fnt, fill="black")
    img1.rectangle([(350, 30), (400, 50)], fill="crimson")
    img1.rectangle([(500, 30), (550, 50)], fill="grey")
    img1.text((410, 32), "NIR", font=fnt, fill="black")
    img1.text((560, 32), "No Data", font=fnt, fill="black")
    ...
    for i in range(n):
        ...
        for j in range(1001):
            color = dplot.iloc[j, i]
            img1.line([(pos, i * 90 + 100), (pos, i * 90 + 115)], fill=color, width=0)
            color = dplot.iloc[j, i + n]
            img1.line([(pos, i * 90 + 116), (pos, i * 90 + 130)], fill=color, width=0)
            pos += 1

        pair_str = ''
        
        for k in range(TRIANG_NO):
            pair_str = pair_str + "-" + dpairs[i][k]
        ...
        img1.text(
            (50, 70 + i * 90),
            f"Chromosome {CHROMOSOME} " + pair_str,
            font=fnt,
            fill="black",
        )
...
def display_chromosomes(pair):
    ...
    img1.text((50, 32), pname, font=fnt1, fill="black")        
    
    return img
    
def colorize(dplot):
                
    lenchr = max(end) - min(begin)
    
    dg = pd.DataFrame(data={"fract": np.arange(0, 1001), 'color':'grey'})
        
    dplot['fract'] = round((dplot['position'] - min(begin))/lenchr,3) * 1000 
    
    dplot = dplot.drop_duplicates(subset=['fract'],keep='first')
    
    dplot = dplot.drop('position', axis = 1)
        
    dplot = pd.concat([dplot, dg])
    
    dplot = dplot.sort_values(by = ['fract','color']) 

    dplot = dplot.drop_duplicates(subset = 'fract', keep='last')
    
    dplot = dplot.set_index('fract')
    
    dplot = dplot.drop('color', axis=1)
    
    dplot = dplot.fillna('grey')
...
def add_bar(dplot):
    ...
    for pair in match_pairs:
        ...
        dplot[pname + '-color'] = 'black'
        
        for i in range(len(dx)):
            if dx.loc[i,'Match Pair'] == pname:
                dplot.loc[dx.loc[i,'Start Fract']:dx.loc[i,'Finish Fract'], 
                             pname + '-color'] = 'blue'
...
if __name__ == "__main__":
    ...
    if 0 < len(INDIVIDUALS) < TRIANG_NO:
        print(f"\nThere must be at least {TRIANG_NO} individuals. Try again.")
        sys.exit()
    ...
    if SUBJECT != '':
        match_pairs = []
        combs = list(combinations(INDIVIDUALS, TRIANG_NO - 1))
        for com in combs:
            if SUBJECT not in com:
                comb = (SUBJECT, *com)
                match_pairs.append(comb)
    else:
        match_pairs = list(combinations(INDIVIDUALS, TRIANG_NO))
    ...
    if SAVE_CHROMS_IMAGES or VIEW_CHROMS_IMAGES:
        for pair in match_pairs:            
            ...
            if SAVE_CHROMS_IMAGES:
                fname = os.path.join(RESULTS_PATH, RES_FILE_NAME) + \
                "_" + pname + ".png"
    ...
    if SAVE_SEP_PAIRS and len(dmatch) > 0:
        for pair in match_pairs:
            ...
            dv = dmatch[dmatch['Match Pair'] == pname]
            
            fname = os.path.join(RESULTS_PATH, RES_FILE_NAME) + "_" + pname + ".csv"
